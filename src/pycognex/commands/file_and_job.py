...
def write_file(filename: str, size: int, data: bytes, checksum: str) -> None:
    """
...
def delete_file(filename: str) -> None:
    ...
    Note:
        ...
        - To delete a job or .CXD file stored in the vision system's RAMDisk folder, the syntax is: DFRAMDisk/[Filename].
          For example, to delete the file "Test.job" stored in the vision system's RAMDisk folder, issue the following command: "DFRAMDisk/Test.job".
        - To delete a job or .CXD file stored in the vision system's SD Card folder, the syntax is: DFSDCARD/[Filename].
          For example, to delete the file "Test.job" stored in the vision system's SD Card folder, issue the following command: "DFSDCARD/Test.job".
...
def get_file() -> str:
    ...
    Note:
        ...
        - If the active job is saved to the vision system's RAMDisk folder, the Get File output includes the RAMDisk folder path.
          For example, if the active job, "Test.job", is saved in the vision system's RAMDisk folder, the Get File command returns "RAMDisk/Test.job".
        - If the active job is saved to the vision system's SD Card folder, the Get File output includes the SD Card folder path.
          For example, if the active job, "Test.job", is saved in the vision system's SD Card folder, the Get File command returns "SDCARD/Test.job".
    ...
    """
    command = "GF"
    status_code, filename = send_command(command)
...
def store_job(job_id: int, job_name: str) -> None:
    """
...
def write_job(job_id: int, job_name: str, job_size: int, job_data: bytes, job_checksum: str) -> None:
    """
...
def get_job() -> int:
    ...
    """
    command = "GJ"
    status_code, job_id = send_command(command)
