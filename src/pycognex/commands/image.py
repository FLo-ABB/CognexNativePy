...
def read_bmp() -> dict:
    ...
    """
    command = "RB"
    status_code, image_data = send_command(command)
...
def write_bmp(image_size: int, image_data: bytes, image_checksum: str) -> None:
    """
...
def write_image(image_size: int, image_data: bytes, image_checksum: str) -> None:
    """
    Sends image data from a remote device to an In-Sight sensor.
